...
- [x] **Faktisk klipp-lengde via ffprobe**: Backend ekstraherer `duration` fra opplastede videoer ved hjelp av `ffprobe` (skriver fil til tempfile, probrer, streamer til object storage). Default `duration_minutes` i ItemModal beregnes fra `Math.ceil(media.duration / 60)` med 5-min minimum, og modalen viser "faktisk klipp-lengde Xm Ys"-hint. ffmpeg/ffprobe re-installert via apt-get (var ikke i fork-containeren). (2026-02-06)
