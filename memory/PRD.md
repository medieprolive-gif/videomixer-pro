...
- [x] **NRK nyhets-ticker på programoversikten** (2026-02-07): Ny `GET /api/news/nrk`-endepunkt henter `https://www.nrk.no/nyheter/siste.rss` via `feedparser` og cacher resultatet 5 min (faller tilbake på stale cache ved RSS-feil). Ny `<NewsTicker>`-komponent rendrer en bunn-stripe inne i `<ProgramOverview>` med statisk rød "NYHETER FRA NRK"-pille til venstre og en CSS-marquee (`@keyframes kk-news-marquee`) som scroller alle overskrifter høyre→venstre. Marqueen bruker dobbel-rendring + dynamisk `--kk-news-distance` målt fra `scrollWidth/2` for sømløs loop. Hastighet 80px/sek. Tickeren vises kun når programoversikten er aktiv.
- [x] **Auto-bumper deaktivert + lyd-lekkasje fikset** (2026-02-07): Bruker rapporterte feilmelding ved overgang programoversikt → bumper, samt lyd som lakk fra kommende innslag mens programoversikten fortsatt var oppe. Tre endringer:
  1. **Backend scheduler**: `scheduler_tick` i `server.py` faller IKKE lenger tilbake på `room.global_bumper_id` når et innslag mangler `pre_plakat_id`. Kun eksplisitt per-innslag-plakat utløser pre-roll.
  2. **Backend WS "ended"**: når et innslag slutter går vi rett til idle/programoversikt — ingen automatisk bumper-mellomstopp før neste schedulerte innslag.
  3. **Display.jsx**: `isInScheduledWindow` ekskluderer global pre-roll-tid (kun item.pre_plakat_duration teller). Apply-state-useEffect returnerer tidlig hvis `showOverviewRef.current=true` slik at den nye videoen ikke blir play()/unmute'et mens overview-overlayen fortsatt vises (eliminerer den brå korte lyd-pulsen). Pause-overview-effect er nå eneste sannhetskilde for play state under overlay.
  Playout-UI viser fortsatt bumper-seksjonen, men dimmet med en `AUTO-BUMPER DEAKTIVERT`-badge slik at funksjonen kan reaktiveres senere uten datatap.
- [x] **Media-kategorisering (content/asset) + lyd-opplasting + bakgrunnsmusikk på programoversikt** (2026-02-07): `videos.category` ("content"|"asset") med tabs på Upload-side. Asset-kategorien holder logoer/bakgrunner/bumpers/musikk utenfor timeline-mediavelgeren. `<audio>`-fil-opplasting støttet (ffprobe gir varighet). Programoversikten har nytt `program_overview_music_id`+`_volume`-felt på room settings; `ProgramOverview` rendrer `<audio autoplay loop>` og volum justeres via slider på Playout. **Bugfix samme dag**: `assetMedia`/`assetMusic` useMemo manglet i Playout-komponenten (var kun definert i ItemModal-scope) → siden krasjet med ReferenceError. Lagt til i Playout-scope.
